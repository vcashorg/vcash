...
impl Foreign {
	...
	pub fn get_token_outputs(
		...
		let output_handler = TokenOutputHandler {
			chain: self.chain.clone(),
		};
		output_handler.get_token_outputs_v2(
			commits,
			token_type,
			start_height,
			end_height,
	...
	pub fn get_unspent_token_outputs(
		...
		output_handler.get_unspent_token_outputs(start_index, end_index, max, include_proof)
	}
