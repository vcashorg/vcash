...
/// Internal function to retrieves an output by a given commitment
fn get_unspent_token(
	chain: &Arc<chain::Chain>,
	id: &str,
	token_type: TokenKey,
) -> Result<Option<(CommitPos, TokenOutputIdentifier)>, Error> {
	let c = util::from_hex(id)
		.map_err(|_| ErrorKind::Argument(format!("Not a valid commitment: {}", id)))?;
	let commit = Commitment::from_vec(c);
	...
	for x in outputs.iter() {
		if let Some(output_pos) = chain.get_token_unspent(x)? {
...
/// Retrieves an token output from the chain given a commitment and token type.
pub fn get_token_output(
	chain: &Weak<chain::Chain>,
	id: &str,
	token_type: TokenKey,
) -> Result<Option<(TokenOutput, TokenOutputIdentifier)>, Error> {
	let chain = w(chain)?;
	let (output_pos, identifier) = match get_unspent_token(&chain, id, token_type)? {
		Some(x) => x,
		None => return Ok(None),
	};
...
/// Retrieves an output from the chain given a commit id (a tiny bit iteratively)
pub fn get_token_output_v2(
	chain: &Weak<chain::Chain>,
	id: &str,
	token_type: TokenKey,
	include_proof: bool,
	include_merkle_proof: bool,
) -> Result<Option<(OutputPrintable, TokenOutputIdentifier)>, Error> {
	let chain = w(chain)?;
	let (output_pos, identifier) = match get_unspent_token(&chain, id, token_type)? {
		Some(x) => x,
		None => return Ok(None),
	...
	let output = chain.get_unspent_token_output_at(output_pos.pos)?;
	let header = if include_merkle_proof && output.is_tokenissue() {
		chain.get_header_by_height(output_pos.height).ok()
	} else {
		None
	};

	let output_printable = OutputPrintable::from_token_output(
		&output,
		&chain,
		header.as_ref(),
		include_proof,
