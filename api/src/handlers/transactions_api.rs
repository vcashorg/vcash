...
impl TxHashSetHandler {
	...
	fn get_last_n_token_output(&self, distance: u64) -> Result<Vec<TxHashSetNode>, Error> {
		let chain = w(&self.chain)?;
		Ok(TxHashSetNode::get_last_n_token_output(&chain, distance))
	}

	// gets last n token outputs inserted in to the tree
	fn get_last_n_token_rangeproof(&self, distance: u64) -> Result<Vec<TxHashSetNode>, Error> {
		let chain = w(&self.chain)?;
		Ok(TxHashSetNode::get_last_n_token_rangeproof(&chain, distance))
	}
	...
	fn token_outputs(
		...
		let out = OutputListing {
			...
			outputs: outputs
				.2
				.iter()
				.map(|x| OutputPrintable::from_token_output(x, &chain, None, true, true))
				.collect::<Result<Vec<_>, _>>()
