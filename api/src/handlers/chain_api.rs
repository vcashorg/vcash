...
use super::utils::{get_output, get_output_v2, get_token_output, get_token_output_v2, w};
use crate::chain;
...
impl TokenOutputHandler {
	...
	pub fn get_token_outputs_v2(
		...
		if let Some(start_height) = start_height {
			if let Some(end_height) = end_height {
				let block_output_batch = self.token_outputs_block_batch_v2(
					token_type,
					start_height,
					end_height,
					include_proof.unwrap_or(false),
	...
	// allows traversal of utxo set
	pub fn get_unspent_token_outputs(
		&self,
		start_index: u64,
		end_index: Option<u64>,
		mut max: u64,
	...
	fn token_outputs_at_height_v2(
		...
		let outputs = outputs
			.into_iter()
			.filter(|outputprint| {
				if outputprint.token_type.is_some() {
					if outputprint.token_type.clone().unwrap() == token_type {
						return true;
					}
				}
				false
	...
	// returns token outputs for a specified range of blocks
	fn token_outputs_block_batch_v2(
		&self,
		token_type: String,
		start_height: u64,
		end_height: u64,
		...
		debug!(
			"token_outputs_block_batch_v2: {}-{}, {}, {}",
			start_height, end_height, include_rproof, include_merkle_proof,
		);

		let mut return_vec: Vec<OutputPrintable> = vec![];
		for i in (start_height..=end_height).rev() {
			if let Ok(res) = self.token_outputs_at_height_v2(
				token_type.clone(),
				i,
				commitments.clone(),
				include_rproof,
