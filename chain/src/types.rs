...
pub enum SyncStatus {
	...
	/// Validating the token kernels
	TxHashsetTokenKernelsValidation {
		/// kernels validated
		kernels: u64,
		/// kernels in total
		kernels_total: u64,
	...
	/// Validating the range proofs
	TxHashsetTokenRangeProofsValidation {
		/// range proofs validated
		rproofs: u64,
		/// range proofs in total
		rproofs_total: u64,
...
impl TxHashsetWriteStatus for SyncState {
	...
	fn on_validation_token_kernels(&self, kernels: u64, kernels_total: u64) {
		self.update(SyncStatus::TxHashsetTokenKernelsValidation {
			kernels,
			kernels_total,
		});
	}

	fn on_validation_token_rproofs(&self, rproofs: u64, rproofs_total: u64) {
		self.update(SyncStatus::TxHashsetTokenRangeProofsValidation {
			rproofs,
			rproofs_total,
		});
	}
...
impl TxHashSetRoots {
	...
	pub fn validate(&self, header: &BlockHeader) -> Result<(), Error> {
		...
		} else if header.token_output_root != self.token_output_root {
			Err(ErrorKind::InvalidRoot.into())
		} else if header.token_range_proof_root != self.token_rproof_root {
			Err(ErrorKind::InvalidRoot.into())
		} else if header.token_issue_proof_root != self.token_issue_proof_root {
			Err(ErrorKind::InvalidRoot.into())
		} else if header.token_kernel_root != self.token_kernel_root {
			Err(ErrorKind::InvalidRoot.into())
		} else {
			Ok(())
		}
...
pub trait TxHashsetWriteStatus {
	...
	/// Starting token kernel validation
	fn on_validation_token_kernels(&self, kernels: u64, kernel_total: u64);
	/// Starting token rproof validation
...
impl TxHashsetWriteStatus for NoStatus {
	...
	fn on_validation_token_kernels(&self, _ks: u64, _kts: u64) {}
	fn on_validation_token_rproofs(&self, _rs: u64, _rt: u64) {}
	fn on_save(&self) {}
	fn on_done(&self) {}
}
