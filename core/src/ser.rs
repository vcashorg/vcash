...
impl<'a> Reader for BinReader<'a> {
	...
	fn read_u16_le(&mut self) -> Result<u16, Error> {
		self.source.read_u16::<LittleEndian>().map_err(Into::into)
	}
	fn read_u32(&mut self) -> Result<u32, Error> {
		self.source.read_u32::<BigEndian>().map_err(Into::into)
	}
	fn read_u32_le(&mut self) -> Result<u32, Error> {
		self.source.read_u32::<LittleEndian>().map_err(Into::into)
	}
	fn read_i32(&mut self) -> Result<i32, Error> {
		self.source.read_i32::<BigEndian>().map_err(Into::into)
	}
	fn read_i32_le(&mut self) -> Result<i32, Error> {
		self.source.read_i32::<LittleEndian>().map_err(Into::into)
	}
	fn read_u64(&mut self) -> Result<u64, Error> {
		self.source.read_u64::<BigEndian>().map_err(Into::into)
	}
	fn read_u64_le(&mut self) -> Result<u64, Error> {
		self.source.read_u64::<LittleEndian>().map_err(Into::into)
	}
	fn read_i64(&mut self) -> Result<i64, Error> {
		self.source.read_i64::<BigEndian>().map_err(Into::into)
	}
	fn read_i64_le(&mut self) -> Result<i64, Error> {
		self.source.read_i64::<LittleEndian>().map_err(Into::into)
	}
